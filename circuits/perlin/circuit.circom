...
include "./quin_selector/circuit.circom";
...
template GetCornersAndGradVectors(scale_bits, DENOMINATOR, SQRT_P) {
    ...
    component xmodulo = Modulo(100);
    xmodulo.in <== p[0];
    xmodulo.divider <== scale;
    bottomLeftCoords[0] <== p[0] - xmodulo.out;

    component ymodulo = Modulo(100);
    ymodulo.divider <== scale;
    ymodulo.in <== p[1];
    bottomLeftCoords[1] <== p[1] - ymodulo.out;
...
template GetWeight(cornerIndex, DENOMINATOR) {
    signal input corner[2];
    signal input p[2];
...
template PerlinValue(DENOMINATOR) {
    ...
    for (var i = 0; i < 4; i++) {
        getWeights[i] = GetWeight(i, DENOMINATOR);
        getWeights[i].corner <== coords[i];
        getWeights[i].p <== p;
    }
  ...
  for (var i = 0; i < 4; i++) {
        ...
        tempCorner0[i][0] <-- coords[i][0] / scale;
        coords[i][0] === tempCorner0[i][0] * scale;

        // getWeights[i].corner[1] <-- coords[i][1] / scale;
        tempCorner0[i][1] <-- coords[i][1] / scale;
        coords[i][1] === tempCorner0[i][1] * scale;

        // getWeights[i].p[0] <-- p[0] / scale;
        tempCorner0[i][2] <-- p[0] / scale;
        p[0] === tempCorner0[i][2] * scale;

        // getWeights[i].p[1] <-- p[1] / scale;
        tempCorner0[i][3] <-- p[1] / scale;
        p[1] === tempCorner0[i][3] * scale;
...
template MultiScalePerlin() {
    ...
    component divBy16 = Modulo(100);
    divBy16.in <== outDividedByCount * 16;
    divBy16.divider <== DENOMINATOR;
    out <== divBy16.quot + 16;
}
