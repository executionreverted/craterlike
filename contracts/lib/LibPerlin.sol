...
library LibPerlinBySeed {
    ...
    function getGradientAt(
        ...
        uint256 idx = uint256(keccak256(abi.encodePacked(x, y, scale, seed))) %
            16;
    ...
    function getSingleScalePerlin(
        ...
        return
            ABDKMath64x64.divi(
                int256(resNumerator),
                int256(vecsDenom) * int256(int32(scale)) ** 3
            );
    }
    ...
    function computePerlin(
        ...
        perlin = ABDKMath64x64.add(
            perlin,
            getSingleScalePerlin(x, y, scale * uint32(2 ** 0), seed)
        );
